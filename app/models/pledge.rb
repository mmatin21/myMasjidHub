class Pledge < ApplicationRecord
  ...
  before_destroy :nullify_donations
  ...
  def self.group_by_year_to_date
    ...
    pledge_amounts = where(created_at: start_date..end_date)
  ...
  def name
    "#{fundraiser.name} - #{number_to_currency(amount)}"
  end

  private

  def nullify_donations
    donations.update_all(pledge_id: nil)
  end

  def build_new_contact_if_needed
    return unless new_contact_attributes.present?

    self.contact = Contact.new(new_contact_attributes)
  end

  def self.ransackable_attributes(auth_object = nil)
    super + %w[created_at amount]
  end

  def self.ransackable_associations(auth_object = nil)
    super + %w[contact fundraiser]
  end
  ...
  def self.to_csv
    CSV.generate(headers: true) do |csv|
      ...
      all.each do |pledge|
        csv << [pledge.contact.name, pledge.contact.email, pledge.fundraiser.name, "$#{pledge.amount}",
                "$#{pledge.donations.sum(:amount)}"]
      end
    end
  end
