...
class Donation < ApplicationRecord
  ...
  def self.to_csv
    CSV.generate(headers: true) do |csv|
      csv << ["first_name", "last_name", "email", "fundraiser", "amount", "created_at"]
      all.each do |donation|
        csv << [donation.contact.first_name, donation.contact.last_name, donation.contact.email, donation.fundraiser.name, "$#{donation.amount}", donation.created_at.strftime("%m/%d/%Y")]
      end
    end
  end
