class MasjidsController < ApplicationController
  ...
  def show
    ...
    if @masjid.stripe_account_id.nil?
      ...
      begin
        ...
        if balance['available'].any?
          ...
          @available_balance += withdrawal_balance if withdrawal_balance < 0
        else
          @available_balance = 0.0
      ...
      @stripe_onboarding_link = generate_stripe_onboarding_link(@masjid) if onboarding_incomplete?(@masjid)
    end
  ...
  def masjid_params
    params.require(:masjid).permit(:name, :address, :city, :state, :zipcode, :phone_number, :latitude, :longitude)
  end
end
